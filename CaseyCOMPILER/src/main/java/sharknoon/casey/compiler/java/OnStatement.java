...
public class OnStatement {
    ...
    public static CodeBlock accept(CLIArgs args, Statement statement) {
        ...
        switch (statement.type) {
            ...
            case EQUALS:
                ...
                objectsTypeName = ClassName.get(Objects.class);
                return onOperator(args, statement, ", ", " && ", "$T.equals(", ")", objectsTypeName);
    ...
    private static CodeBlock onOperator(CLIArgs args,
        ...
        return onOperator(args, statement, secondaryDelimiter, primaryDelimiter, "", "", null);
    }
    ...
    private static CodeBlock onOperator(CLIArgs args,
        ...
        if (secondaryPrefixType == null) {
            builder.add("((" + secondaryPrefix + "$L" + secondaryDelimiter + "$L" + secondarySuffix + ")", firstParameter, secondParameter);
        } else {
            builder.add("((" + secondaryPrefix + "$L" + secondaryDelimiter + "$L" + secondarySuffix + ")", secondaryPrefixType, firstParameter, secondParameter);
        }
        for (int i = 2; i < parameters.size(); i++) {
            ...
            builder.add(primaryDelimiter + "(" + secondaryPrefix + "$L" + secondaryDelimiter + "$L" + secondarySuffix + ")", secondaryPrefixType, firstParameter, secondParameter);
        }
        ...
        for (Call call : calls) {
            ...
            if (item.item == ItemType.VARIABLE) {
                ...
                for (Statement parameter : call.parameter) {
                    builder.add(accept(args, parameter));
                }
                builder.add(")");
            } else {
