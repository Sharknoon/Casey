 ...
 * @author Josua Frank
 */
public class DivideOperator extends Operator<NumberType, NumberType> {

    public DivideOperator(Statement parent) {
    ...
    public NumberValue calculateResult() {
        ...
        for (int i = 0; i < parameters.size(); i++) {
            ...
            if (par != null) {
                Value<NumberType> val = par.calculateResult();
                double value = Value.toNumberValue(val).getValue();
                if (Double.isNaN(result)) {
                    result = value;
                } else {
                    if (value == 0.0) {
                        return new NumberValue(null);//division through zero is not okay
                    } else {
                        result = result / value;
                    }
                }
